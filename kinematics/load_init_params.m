%% contact params
mu = 0.5;
h1 = trvec2tform([-0.03,  0.03, -0.03]); % position of {C_i} in {B}
h2 = trvec2tform([ 0.03,  0.03, -0.03]);
h3 = trvec2tform([ 0.03, -0.03, -0.03]);
h4 = trvec2tform([-0.03, -0.03, -0.03]);
...
for i = 1 : size(tSteps,2)
    ...
    pd_shifted(i, 1:3) = (zyx2R(od(i,1:3))'*pd(i, 1:3)')';
    w_des_shifted(1:3, i) = (zyx2R(od(i,1:3))'*w_des(1:3,i));
end
%% circular trajectory 
